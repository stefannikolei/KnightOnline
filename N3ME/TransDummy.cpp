...
void CTransDummy::Render()
{
	...
	// set transform
	hr = s_lpD3DDev->SetTransform(D3DTS_WORLD, &m_Matrix); // 월드 행렬 적용.. [Korean comment]
...
BOOL CTransDummy::MouseMsgFilter(LPMSG pMsg)
{
	...
	case WM_LBUTTONDOWN:
		{
			...
			if (m_pSelectedCube)
			{
				...
				for (int i=0; i<iSize; ++i)	// 모든 선택된 객체의 스케일 저장 Save
				{
	...
	case WM_LBUTTONUP:
		...
		break;
	case WM_RBUTTONUP:	// 큐브 선택 취소 및 이번 드래그로 움직인것 되돌려 놓기 Select
		{
			if (m_pSelectedCube)
			{
...
void CTransDummy::TransDiff(__Vector3* pvDiffPos, __Quaternion* pqDiffRot, __Vector3* pvDiffScale)
{
	...
	if (pqDiffRot)
	{
		...
		for(i=0; i<iSize; ++i)
		{
			...
			vPos = pSelObj->Pos();	//	맵상에서의 위치 Position
			vPos -= vCenter;
			vPos *= mtx44Rotate;
			vPos += vCenter;
			pSelObj->PosSet(vPos);
