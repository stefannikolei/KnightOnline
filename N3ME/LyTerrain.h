...
////////////////////////////////////////////////
//	multimap사용	 [Korean comment]
//savegamedata할때 쓸꺼얌.. [Korean comment]
//colormap 찍을때도 써..^^ [Korean comment]
//
...
protected:
	...
	int							m_iEditMode;												// Edit의 모드인가? Mode

	CDTexMng*					m_pDTexMng;

	MMap						m_ColorMapTileTree;	//컬러맵 만들기전 타일들 갖고 있는 트리.. [Korean comment]

	int							m_iDistanceTable[DISTANCE_TABLE_SIZE][DISTANCE_TABLE_SIZE];
	
public:
	float						m_fBrushIntensityMap[MAX_BRUSH_SIZE][MAX_BRUSH_SIZE];	// Brush의 변화율을 표시한 맵 [Korean comment]
	...
	//LightMap관련... [Korean comment]
	CDlgSetLightMap*			m_pDlgSetLightMap;
	LPDIRECT3DVERTEXBUFFER9		m_LightMapVB;	
	CN3Texture***				m_ppLightMapTexture;
	bool**						m_ppIsLightMap;
	int							m_iNumLightMap;
	bool						m_bDrawLineLightMap;	//라이트 맵 그리는 옵션에서.. 라인형태로 그리는지.. [Korean comment]
	__VertexXyzColor			m_vLineLightMap[2];		//라인형태로 그린다면 그 라인을 화면에 표시해야되겠지?..^^ 그점이야.. [Korean comment]

	//LOD관련.. [Korean comment]
	int							m_iHeightLimit;
	int							m_iDistLimit;

	std::list<CQTNode*>			m_RenderNodes;

	LPDIRECT3DVERTEXBUFFER9		m_ColorMapVB;	//	컬러맵쓸때.. [Korean comment]
	LPDIRECT3DVERTEXBUFFER9		m_TileVB;	//	하나의 타일과 하나의 라이트맵 [Korean comment]
	...
	// ColorMap 관련... [Korean comment]
	LPDIRECT3DVERTEXBUFFER9		m_pColorMapTmpVB;
	__VertexTransformedT2*		m_pColorMapTmpVertices;

	//Brush 관련.. [Korean comment]
	BOOL						m_bBrushFlat;	//지형 높이고 내리는 브러쉬를 평평하게 할꺼야? [Korean comment]
...
protected:
	...
	void PutColorMapTile(int x, int z);		//컬러맵 만들 타일 넣기...(아직 안만들어진...그런 타일들 대기열에 넣는것..) [Korean comment]
	...
	void	SetVisibleRect();	//타일단위.. [Korean comment]

	//map editor 관련.. [Korean comment]
	//
	void SetDTexManual(int x, int y);	//수동찍기 [Korean comment]
	void SetDTex(int x, int y);			//자동찍기. [Korean comment]
	BOOL IntersectTriangle( const D3DXVECTOR3 orig, const D3DXVECTOR3 dir, 
						D3DXVECTOR3& v0, D3DXVECTOR3& v1, D3DXVECTOR3& v2, float* t, float* u, float* v);
	void Heighten(POINT ptCenter, float fHeight);	// 지형을 높게하거나 낮게 하는 함수 Function
	void Smooth(POINT ptCenter);		// 지형을 주변 맵의 높이값과 비교해서 부드럽게 한다. [Korean comment]
	void Flaten(POINT ptCenter);
	void SetApexHeight(int x, int z, float fHeight);		// 높이맵의 높이값 지정 [Korean comment]
	float GetApexHeight(int x, int z) const;				// 높이맵의 높이값 얻기 [Korean comment]
	float GetFallOffValue(float fFallOff, float x);		// BrushIntensity 계산할때 쓰이는 식. Calculate

	//개선된 라이트맵관련..(2002. 11. 13..) [Korean comment]
	void ConvertLightMapToolDataV2toV3();
	//
			
public:
	void UpdateBrushArea(POINT ptCenter);				// Brush 영역 표시 vertex갱신
	void RenderBrushArea();								// Brush 영역 표시 그리기 Draw
	...
	void	TilingAll(); //지형 전체를 선택된 타일로 깔기.. Select
	...
	BOOL MouseMsgFilter(LPMSG pMsg);	// 지형 고칠때 마우스 메세지 처리 Process
