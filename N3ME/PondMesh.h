...
protected:
	...
	struct __SELECT_PO
	{
		int		ix;	//	m_iWaterScaleWidth 상의 숫자 [Korean comment]
		int		iz;	//	m_iWaterScaleHeight 상의 숫자 [Korean comment]
	};

	typedef std::list<__SELECT_PO*>::iterator it_SelVtx;
	std::list<__SELECT_PO*>			m_vSelect;	//	선택된 점들의 관리 Select

	//	기본 데이타 [Korean comment]
	int				m_iPondID;
	float			m_fWaterHeight;	//	물높이 [Korean comment]
	int				m_iWaterScaleWidth;	//	가로방향의 점의 갯수 [Korean comment]
	int				m_iWaterScaleHeight;//	세로방향의 점의 갯수 [Korean comment]
	DWORD			m_dwPondAlpha;		//	물의 알파 [Korean comment]
	float			m_fTU;
	float			m_fTV;
	float			m_fWaterScaleX,m_fWaterScaleZ;	//	정렬했을시 점간의 거리 [Korean comment]

	int				m_iBackUpWidht;
	int				m_iBackUpHeight;

	__Vector3		m_pVertices[MAX_PONDMESH_VERTEX];	//	저장,백업용으로 쓰이는 값 Save
	__VertexXyzT2	m_pViewVts[MAX_PONDMESH_VERTEX];	//	화면에 뿌려지는 값 [Korean comment]
	...
	//	연못영역에 관한 데이타 [Korean comment]
	__Vector3		m_vDrawBox[4];		//	화면상에 나올 영역(왼쪽위부터 시계방향으로) [Korean comment]
	__VertexXyzColor m_ViewRect[5];		//	화면에 그릴선 [Korean comment]
	int				m_iRectVC;
	__VertexXyzT2	m_pRectVts[100];	//	영역줄에 그려질 점들 [Korean comment]

	__Vector3		m_vSelectBox[2];	//	점들을 선택했을시 점들의 영역(점들이 화면영역을 벋어났을시) Select
...
public:
	...
	void			MakeDrawRect(__Vector3* pvPos);	//	화면에 보일 연못의 영역테두리 만든다 [Korean comment]
	void			UpdateDrawPos();		//	영역줄에 그려질 점들  [Korean comment]

	void			UpdateWaterHeight();	//	현재의 물의 높이로 세팅 [Korean comment]
	void			UpdateMovePos(__Vector3 vMovingPos);	//	이동 Move

	void			MakePondPos();	//	현재 박스에서 연못을 만든다 [Korean comment]
	void			MakeIndex();
	void			ReCalcUV();		//	점에 상관없이 평편하게 뿌린다 [Korean comment]
	void			ReCalcVexUV();	//	점에 관여하여 뿌린다 [Korean comment]

	void			EstimatePos();	//	줄의 크기에 따라 점들의 갯수를 임의로 지정//// Size

	void			ClearSelectPos();	//	선택점에 관한 데이터 초기화 Initialize
	void			InputSelectPos(float fX,float fY,float fZ,int iVC=-1);	//	선택한 점들에 대한 자료입력 Select
	BOOL			InputDummyMovingPos(__Vector3 vDummyMovingPos,BOOL bMovePond);	//	더미가 움직였을시 나머지점을 처리하기 위해 Process

protected:
	void			MovingPos();
	void			CalcuWidth(int iSx,int iSy,int iEx=-1,int iEy=-1);	//	처음 좌우로의 계산을 한다 Calculate
	void			SetAllPos(int iSx,int iSy,int iEx=-1,int iEy=-1);	//	처음 계산한점들외에 점들 좌표 계산 Calculate
	void			ReInputBackPos();	//	새로 옮긴좌표를 백업에 넣어줌  [Korean comment]

public:	//	다이얼로그 상자에서 쓰이는 함수 Function
