...
public:
	...
	bool			m_bViewWireFrame; // 선택된 오브젝트를 와이어 프레임으로 보게 한다.. Object
	bool			m_bRenderAxisAndGrid;			// 축과 그리드를 그릴지 안그릴지 결정.. [Korean comment]

	int				m_iZoneID; // 존 아이디... [Korean comment]
	bool			m_bHideObj;
	bool			m_bLoadingComplete; // 이게 참이어야만 렌더링을 한다.. Rendering

	CMainFrame*			m_pMainFrm;						// Main 윈도우 Window
	CSowSeedMng			m_SowSeedMng;
	CDlgShapeList*		m_pDlgSourceList;			// Object 목록을 보여줄 다이알로그 [Korean comment]
...
protected:
	...
	CEventMgr*			m_pEventMgr;				//존 이벤트 관리... [Korean comment]
	CWallMgr*			m_pWall;					//존 외각 가지 못하는 벽관리.. [Korean comment]
	CNPCPathMgr*		m_pNPCPath;					// NPC 길 만드는 클래스.. [Korean comment]
	CLyTerrain*			m_pTerrain;						// 지형 [Korean comment]
	CN3Scene*			m_pSceneSource;					// source object 목록에 보여줄 것들을 담은 Scene
	CN3Scene*			m_pSceneOutput;					// 맵에 배치한 Object를 포함한 Scene
	
	//CDlgSceneGraph*		m_pDlgSourceList;			// Object 목록을 보여줄 다이알로그 [Korean comment]
	//CDlgSceneGraph*		m_pDlgOutputList;			// 맵에 배치한 Object를 보여줄 다이알로그
	CDlgShapeList*		m_pDlgOutputList;			// 맵에 배치한 Object를 보여줄 다이알로그
	CDlgBase*			m_pDlgBase;						// 객체 등록정보 편집 대화상자.. Info
	CBrushDlg*			m_pBrushDlg;					// 지형 편집 브러쉬 옵션지정해주는 대화상자 Edit
	

	CN3Transform*		m_pSelSourceObj;				// source object 목록에서 선택한 것 Select
//	CTypedPtrArray<CPtrArray, CN3TransformCollision*> m_SelOutputObjArray;	// 맵에 배치한 Object 중 선택한 것들
	...
	int					m_CursorMode;					// 마우스의 기능 모드 구분 Mode
	BOOL				m_bRenderSelObj;				// source object 목록에서 선택된 것을 그릴지 안그릴지 결정 Select

	CTransDummy*		m_pDummy;						// 물체를 이동 회전 확대/축소 하는 기능을 담당하는 클래스 Move
	CPosDummy			m_PosDummy;						// 이동 Move
	CRotDummy			m_RotDummy;						// 회전 Rotate
	CScaleDummy			m_ScaleDummy;					// 확대/축소 [Korean comment]
	CRiverMng			m_RiverMng;						// 강물 정보관리 Info
	CPondMng			m_PondMng;						// 연못 정보관리 Info

	RECT				m_rcSelDrag;					// 드래그 영역 [Korean comment]

	struct __Sort										// Object picking할때 정렬하는 구조체 [Korean comment]
	{
...
public:
	...
	void	SetZoneID(int id) { m_iZoneID = id; if(m_pTerrain) m_pTerrain->m_iZoneID = id; }	//지형 존 아이디 셋팅. [Korean comment]
	void	ImportShape(const char* szFullPath);
	void	MakeTerrainMovableAttr(CN3ShapeMgr* pShapeMgr);		//지형에서 갈수 있는 타일과 갈 수 없는 타일을 정리해라.. [Korean comment]
	void	ImportPostDataFromScene(const char* szFileName); // Scene 에서 오브젝트 배치된걸 불러온다.. Object
	void	UpDateFP(); 
	void	Tick();
	void	Render();
	void	Release();
	void	SavePartition(float x, float z, float width);
	void	SaveToFile(LPCTSTR lpszPathName);			// Map 파일 저장 Save
	void	LoadFromFile(LPCTSTR lpszPathName);			// Map 파일 불러오기 File
	BOOL	MouseMsgFilter(LPMSG pMsg);			// 마우스의 기능 [Korean comment]
	void	ImportTerrain(const char* szMeshFN);			// VMesh 파일에서 지형 데이터 읽어오기 Data
	void	ImportTerrainHeight(const char* szMeshFN);		// VMesh 파일에서 지형의 높이값만 읽어오기.. File
	void	MakeGameFiles(LPCTSTR lpszPathName, float fSize = 128.0f);	// 게임 데이터로 변환하기 Data
	void	MakeServerDataFiles(LPCTSTR lpszPathName);
	void	SelectObject(CN3Base* pObj, BOOL IsSourceObj, BOOL bAdd = FALSE);	// 객체를 선택한다. Select
	void	RenderObjectToWindow(CN3TransformCollision* pObj, HWND hWnd);	// 특정 윈도우에 Object를 그려준다.
	void	SetCursorMode(int iMode);						// 마우스커서의 이용방법을 바꾼다. [Korean comment]
	int		GetCursorMode() const {return m_CursorMode;}	// 마우스 커서의 이용방법을 얻는다. [Korean comment]
	void	Invalidate();									// View 화면 갱신 [Korean comment]
	void	FocusSelObj();									// 선택된 객체로 포터스를 옮긴다. Select
	void	FocusAll();										// 전체 맵을 볼수 있도록 한다. [Korean comment]
	void	FocusAt(__Vector3 v);							// 지정된 점으로 포커스 옮겨. [Korean comment]
	void	DropSelObjToTerrain();							// 선택한 객체를 지형에 붙인다.(Y값만 조정)
	void	DeleteSelObjectFromOutputScene();			// OutputScene에서 선택한 객체를 지운다. Select
	void	RenderDragRect(RECT* rc);					// 드래그 영역을 그린다. [Korean comment]
	...
	void	OnSelChanged();								// 선택한 객체가 바뀌었을때 해줄것들 Select
	void	SelectObjectByDragRect(RECT* pRect, BOOL bAdd);	// 드래그 해서 객체 선택하기 Select
...
//	BOOL	CameraMove(LPMSG pMsg);		// 카메라 이동에 관한 메세지 처리 Process
	BOOL	GetObjectMinMax(CN3Transform* pObj, __Vector3& vMin, __Vector3& vMax);	// 객체의 최대최소점 찾기 [Korean comment]
