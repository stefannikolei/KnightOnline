...
//	- 게임에 쓰는 지형 (쿼드트리 아님...^^) [Korean comment]
//	- 2001. 10. 22.
//
//	By Donghoon..
//
...
const int PATCH_PIXEL_SIZE = 32;	//	패치하나에 들어가는 픽셀수.. [Korean comment]
const int UNITUV = 32;				//	컬러맵 텍스쳐 한장에 들어가는 타일수.. [Korean comment]
const int LIGHTMAP_TEX_SIZE = 16;	//	타일 하나당 들어가는 라이트맵 텍스쳐 크기. Size
const int TILE_PIXEL_SIZE = 128;	//	타일하나의 실제 픽셀 사이즈. [Korean comment]
...
typedef struct __MapData			// 맵데이터... Data
{
	float	fHeight;				//타일텍스쳐가 꽉차는지 아닌지.. [Korean comment]
	uint32_t	bIsTileFull	: 1;		//지형의 높이값.. [Korean comment]
	uint32_t	Tex1Dir		: 5;		//첫번째 타일 텍스쳐 찍는 방향. [Korean comment]
	uint32_t	Tex2Dir		: 5;		//두번째 타일 텍스쳐 찍는 방향. [Korean comment]
	uint32_t	Tex1Idx		: 10;		//첫번째 타일 텍스쳐의 인덱스. Index
	uint32_t	Tex2Idx		: 10;		//두번째 타일 텍스쳐의 인덱스. Index
