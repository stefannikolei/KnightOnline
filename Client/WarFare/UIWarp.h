...
struct __WarpInfo
{
	int iID; // 이 번호를 서버에게 날린다..  [Korean comment]
	std::string szName;
	std::string szAgreement; // 유저한테 알리고 동의를 구하는 글.. [Korean comment]
	int iZone;
	int iMaxUser; // 최대 제한 인원수.. [Korean comment]
	int iGold;
	__Vector3 vPos;
};
...
protected:
	...
	class CN3UIString*		m_pText_Agreement; // 동의 사항.. [Korean comment]
