...
// 카메라 시점 [Korean comment]
enum e_ViewPoint		{	VP_BACKWARD = 0,			// 플레이어 등 뒤에서 바라본 모습 1.5인칭 [Korean comment]
							VP_FIRST_PERSON = 1,		// 일인칭 [Korean comment]
							VP_FOWARD = 2,				// 플레이어 앞에서 바라봄.. [Korean comment]
							VP_THIRD_PERSON = 3,		// 삼인칭 모드 .. 마우스도 동작함.. Mode
							VP_UNKNOWN = 0xffffffff };  // 개발을 위해 카메라만 빠르게.... [Korean comment]
...
protected:
	...
	float			m_fFPDeltaCur; // 현재  [Korean comment]
	float			m_fFPDeltaToReach; // 이값을 목표로 해서 변한다. [Korean comment]

	float			m_fRotPitchBackward; // 1.5인칭 시점일때 카메라 피치 각도. [Korean comment]
	float			m_fRotPitchFirstPerson; // 일인칭 시점일때 카메라 피치 각도. [Korean comment]
	float			m_fRotPitchFoward; // 앞에서 본 시점일때  카메라 피치 각도. [Korean comment]
	float			m_fRotPitchThirdFirson; // 삼인칭 시점 카메라 피치 각도 [Korean comment]
	
	float			m_fOffsetVPGod; // 위에서 바라본 시점일 경우에 거리.. [Korean comment]
	float			m_fRotYawVPGod; // 위에서 바라본 시점일 경우에 카메라 회전각도.. Rotate

	float			m_fZoomBackwardOrFoward; // 1.5인칭, 앞에서 본 시점일때 카메라 Zoom 1.0f 가 기준이다.
	float			m_fLightningTimeRemain; // 번개 칠때 타이머.. [Korean comment]

	e_ViewPoint		m_eViewPoint; // 카메라 시점. [Korean comment]

public:
	void			SetDefaultLight(CN3Light* pSun, CN3Light* pSupport, CN3Light* pCam);
	void			RestoreLighting();
	void			BackupReleaseLighting();
	void			Lightning(); // 번개 치며 라이트 조절하기. [Korean comment]
	...
	e_ViewPoint		ViewPoint() { return m_eViewPoint; } // 시점.. [Korean comment]
	void			CameraPitchAdd(float fRotXPerSec);
	void			CameraYawAdd(float fRotYPerSec);
	void			CameraZoom(float fDelta);
	float			CameraYaw() { return m_fRotYawVPGod; } // 위에서 바라본 시점일 경우에 카메라 회전각도.. Rotate
	void			SetActiveCamera(CN3Camera* pCamera) { m_pActiveCam = pCamera; }
	void			Tick(const D3DCOLOR* crDiffuses,			// Diffuse 라이트 색깔.. Color
						const D3DCOLOR* crAmbients,			// Ambient 라이트 색깔.. Color
	...
	void			ApplyCameraAndLight(); // 카메라와 라이트에 세팅된 값 적용. 라이트는 해의 각도에 따라 다르게 한다. [Korean comment]
