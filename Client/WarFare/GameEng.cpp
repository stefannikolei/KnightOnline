...
//	m_eViewPoint = VP_BACKWARD; // 기본은 1.5인칭(플레이어 뒤에서 바라본 모습)으로 시작. [Korean comment]
	m_eViewPoint = VP_THIRD_PERSON; // 기본은 3인칭으로 시작. [Korean comment]
	m_fRotPitchFirstPerson = 0.0f; // 일인칭 시점일때 카메라 피치 각도. [Korean comment]
	m_fRotPitchBackward = 0.0f; // 삼인칭 시점일때 카메라 피치 각도. [Korean comment]
	m_fRotPitchFoward = 0.0f; // 앞에서 본 시점일때  카메라 피치 각도. [Korean comment]
	m_fRotPitchThirdFirson = D3DXToRadian(25.0f);
	m_fOffsetVPGod = 7.0f; // 위에서 본시점일때 카메라 거리.. [Korean comment]
	m_fRotYawVPGod = 0.0f; // 위에서 바라본 시점일 경우에 카메라 회전각도.. Rotate
	m_fZoomBackwardOrFoward = 1.0f; // 1.5인칭, 앞에서 본 시점일때 카메라 Zoom 1.0f 가 기준이다.
	// 기본 카메라 세팅.. [Korean comment]
	///////////////////////////////////////////////////////////////

	m_fLightningTimeRemain = 0.0f; // 번개 칠때 타이머.. [Korean comment]
	...
	*/
	...
	m_fFPDeltaCur = 1.0f; // 현재  [Korean comment]
	m_fFPDeltaToReach = 1.0f; // 이값을 목표로 해서 변한다. [Korean comment]

//	m_fLightDeltaCur = 1.0f;
//	m_fLightDeltaToReach = 1.0f; // 이값을 목표로 해서 변한다. [Korean comment]
}
...
void CGameEng::Tick(const D3DCOLOR* crDiffuses,			// Diffuse 라이트 색깔.. 3 개 쓴다. Color
					const D3DCOLOR* crAmbients,			// Ambient 라이트 색깔.. 3 개 쓴다. Color
					const D3DCOLOR crFog,				// 안개 색깔.. Color
					const __Vector3& vPosPlayer,		// 플레이어 위치 Position
					const __Quaternion& qtPlayer,		// 회전 쿼터니언 Rotate
					float fHeightPlayer,				// 키를 인수로 넣으면 카메라와 라이트 처리.. Process
					float fSunRadianZ)					// 해의 Z 각도..
{
	...
	////////////////////////////////////////////////////////////////////////////////////
	// 카메라 충돌 체크... Check
	if(VP_FIRST_PERSON == m_eViewPoint) // 일인칭때는 충돌체크 안한다. Check
	{
		m_pActiveCam->LookAt(m_vEyeToReach, m_vAtToReach, __Vector3(0,1,0)); // 처다본다.. [Korean comment]
	}
	else
	{
		__Vector3 vEyeResult = m_vEyeToReach;
		float fNP = m_pActiveCam->m_Data.fNP;
		CGameBase::ACT_WORLD->CheckCollisionCameraWithTerrain(vEyeResult, m_vAtToReach, fNP); // 지형과 충돌체크 Check
		CGameBase::ACT_WORLD->CheckCollisionCameraWithShape(vEyeResult, m_vAtToReach, fNP); // 오브젝트와 충돌체크.. Check
		m_pActiveCam->LookAt(vEyeResult, m_vAtToReach, __Vector3(0,1,0)); // 처다본다.. [Korean comment]
	}
	// 카메라 충돌 체크... Check
	////////////////////////////////////////////////////////////////////////////////////


	// 파 플레인 값을 조정.. [Korean comment]
	// ApplyCameraAndLight 에서 실제로 안개등의 값을 조절한다. [Korean comment]
	if(m_fFPDeltaCur != m_fFPDeltaToReach)
	{
		float fFPChange = (m_fFPDeltaToReach - m_fFPDeltaCur) * s_fSecPerFrm / 5.0f; // 5초동안 변하게 한다. [Korean comment]
	...
	m_pActiveCam->m_FogColor = crFog; // 안개색을 맞춘다.. Color
	m_pActiveCam->Tick(); // 적용및 사면체등등의 값들을 계산.. Calculate

	__Matrix44 mtxRotSun;
	mtxRotSun.RotationZ(fSunRadianZ); // 해의 각도에 맞춘다.. [Korean comment]
	...
	for(int i = 0; i < iSize; i++, itLgt++)
	{
		...
		if(0 == pLight->m_Data.nNumber) // 기본 디렉셔널 라이트 [Korean comment]
		{
			// View Matrix 각도와 방향을 맞춘다.. [Korean comment]
...
//			pLight->m_Data.Direction = vDir * mtxVI;
		...
		{
			__Vector3 vDir(2,-3, 2); // 위에서 아래로 ... [Korean comment]
			vDir.Normalize();
			pLight->m_Data.Direction = vDir;
		...
		else if(2 == pLight->m_Data.nNumber)
		{
			...
			vPos.y += 16.0f;
			pLight->PosSet(vPos); // 카메라 위에 가게 한다.. [Korean comment]
	...
	*/
	...
	if(m_pRefLightCam)
	{
		__Vector3 vPos = s_CameraData.vEye;
		vPos.y += 16.0f;
		m_pRefLightCam->PosSet(vPos); // 카메라 위에 가게 한다.. [Korean comment]
...
void CGameEng::ApplyCameraAndLight()
{
	if(m_pActiveCam)
	{
		m_pActiveCam->Apply(); // 위의 거 대신에 하나만 쓴다... [Korean comment]
	}
/*
...
void CGameEng::ViewPointChange(e_ViewPoint eVP)
{
	if(eVP < VP_BACKWARD || eVP > VP_THIRD_PERSON)
	{
		...
		m_eViewPoint = (e_ViewPoint)iVP;						// 시점 증가.. [Korean comment]
	}
	else
	{
		m_eViewPoint = eVP;						// 시점 증가.. [Korean comment]
	}
}
...
void CGameEng::CameraPitchAdd(float fRotXPerSec)
{
	...
	if(VP_BACKWARD == m_eViewPoint)
	{
		...
		if(m_fRotPitchBackward > fPitchMax) m_fRotPitchBackward = fPitchMax; // 아래 보는값 [Korean comment]
		else if(m_fRotPitchBackward < fPitchMin) m_fRotPitchBackward = fPitchMin; // 위보는 값.. [Korean comment]
	}
	else if(VP_FIRST_PERSON == m_eViewPoint)
	{
		m_fRotPitchFirstPerson += fRotXPerSec * s_fSecPerFrm;
		if(m_fRotPitchFirstPerson > fPitchMax) m_fRotPitchFirstPerson = fPitchMax; // 아래 보는값 [Korean comment]
		else if(m_fRotPitchFirstPerson < fPitchMin) m_fRotPitchFirstPerson = fPitchMin; // 위보는 값.. [Korean comment]
	}
	else if(VP_FOWARD == m_eViewPoint)
	{
		m_fRotPitchFoward += fRotXPerSec * s_fSecPerFrm;
		if(m_fRotPitchFoward > fPitchMax) m_fRotPitchFoward = fPitchMax; // 아래 보는값 [Korean comment]
		else if(m_fRotPitchFoward < fPitchMin) m_fRotPitchFoward = fPitchMin; // 위보는 값.. [Korean comment]
	}
	else if(VP_THIRD_PERSON == m_eViewPoint) // 위에서 보는 건 거리만 늘였다 줄였다 한다.. [Korean comment]
	{
		fPitchMin = D3DXToRadian(10.0f); // 젤 밑에값 한계 [Korean comment]

		m_fRotPitchThirdFirson += fRotXPerSec * s_fSecPerFrm;
		if(m_fRotPitchThirdFirson > fPitchMax) m_fRotPitchThirdFirson = fPitchMax; // 아래 보는값 [Korean comment]
		else if(m_fRotPitchThirdFirson < fPitchMin) m_fRotPitchThirdFirson = fPitchMin; // 위보는 값.. [Korean comment]
	}
}

void CGameEng::CameraYawAdd(float fRotYPerSec)
{
	if(VP_THIRD_PERSON == m_eViewPoint) // 위에서 보는 건 .. [Korean comment]
	{
		m_fRotYawVPGod += fRotYPerSec * s_fSecPerFrm; // 위에서 바라본 시점일 경우에 카메라 회전각도.. Rotate
		return; // 돌아간다! [Korean comment]
	}
}
...
void CGameEng::CameraZoom(float fDelta)
{
	...
	if(VP_BACKWARD == m_eViewPoint || VP_FOWARD == m_eViewPoint)
	{
		...
		if(m_fZoomBackwardOrFoward < 0.4f) m_fZoomBackwardOrFoward = 0.4f; // 아래 보는값 [Korean comment]
	...
	else if(VP_THIRD_PERSON == m_eViewPoint) // 위에서 보는 건 거리만 늘였다 줄였다 한다.. [Korean comment]
	{
		...
		if(m_fOffsetVPGod < 2.0f) m_fOffsetVPGod = 2.0f; // 너무작아지면 멈추고.. [Korean comment]
		else if(m_fOffsetVPGod > 10.0f) m_fOffsetVPGod = 10.0f; // 너무 커져도 멈추고.. [Korean comment]
		return; // 돌아간다! [Korean comment]
	}
}
...
{
	m_fLightningTimeRemain = LIGHTNING_DURATION; // 번개 칠때 타이머.. [Korean comment]
}
