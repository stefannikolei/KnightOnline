...
protected:
	...
	float		m_fRadius;		// 반경 [Korean comment]

//	__Vector3	m_vVelocity;	// 현재 속도(스피드+방향) [Korean comment]
	float		m_fRadianSpeed;		// 속력 [Korean comment]
	float		m_fRadian;		// 현재 타원의 위치각 Position
	float		m_fFactor1,	m_fFactor2;
	float		m_fFactorSpeed1,	m_fFactorSpeed2;

	float		m_fRadiusY;		// 높이 반경 [Korean comment]
	float		m_fFactorY1, m_fFactorY2;	// 높이 변화 요소 [Korean comment]
	float		m_fFactorYSpeed1, m_fFactorYSpeed2;	// 높이 변화 요소 변화율.. [Korean comment]
