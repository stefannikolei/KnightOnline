...
//	// Number of lights in scene
	...
	// Has dummy object data (helper objects not needed in actual game, only during development).
	BOOL bExportCharacter;	// 도우미 오브젝트(??? - 실제 겜에서는 필요없고 개발시에만 필요한 오브젝트를 말한다)데이터를 갖고 있다. 
	// Has character data (helper objects not needed in actual game, only during development). 
	
	BOOL bExportSelectedOnly; // 선택된 것만 ???
	// Export selected only ???

	BOOL bGenerateFileName; // 파일 이름을 0_0000_00_0 포맷으로 바꾼다..??
	// Change file name to 0_0000_00_0 format..??
	BOOL bGenerateSmoothNormal; // 부드럽게 보이도록 법선 벡터들을 재 계산한다.
	// Recalculate normal vectors for smooth appearance.
...
//	// Progressive Mesh generation
	...
	// Automatically optimize texture files and save as "OBM" bitmap files in power-of-2 units to match Direct3D format.
	BOOL bGenerateCompressedTexture; // Texture 압축 사용
	// Use texture compression

	char szSubDir[_MAX_DIR];		// export 할때 저장하는 sub폴더를 지정해준다.
	// Specify subfolder to save when exporting.
};
