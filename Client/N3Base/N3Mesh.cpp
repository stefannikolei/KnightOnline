...
CN3Mesh::CN3Mesh()
{
	...
	m_psnIndices = nullptr; // 인덱스 버퍼 Index
}
...
bool CN3Mesh::Load(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_nVC, 4, &dwRWC, nullptr); // 점갯수 읽기.. Load
	if(m_nVC > 0)
	{
		this->Create(m_nVC, 0); // Vertex Buffer 생성 및 데이터 채우기 Create
		ReadFile(hFile, m_pVertices, m_nVC * sizeof(__VertexT1), &dwRWC, nullptr);
		this->FindMinMax(); // 최대, 최소값을 찾는다. [Korean comment]
	}
	
	ReadFile(hFile, &m_nIC, 4, &dwRWC, nullptr); // 인덱스 갯수 읽기.. Load
	if(m_nIC > 0)
	{
		this->Create(0, m_nIC); // 인덱스 버퍼 생성 및 데이터 채우기 Create
...
bool CN3Mesh::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, &m_nIC, 4, &dwRWC, nullptr); // 인덱스 갯수 읽기.. Load
	if(m_nIC > 0)
	{
...
{
	if(nVC > 0) // 점이 있으면... [Korean comment]
	{
#ifdef _N3GAME
		if (nVC > 32768)
		...
		m_pVertices = new __VertexT1[nVC]; memset(m_pVertices, 0, nVC * sizeof(__VertexT1)); // Vertex Buffer 생성 Create
		m_nVC = nVC;
	}

	if(nIC > 0) // Mesh 로딩에 성공하고, 인덱스가 있으면.. Index
	{
#ifdef _N3GAME
		if (nIC > 32768)
		...
		m_psnIndices = new uint16_t[nIC]; memset(m_psnIndices, 0, nIC * 2); // Index Buffer 생성 Create
		m_nIC = nIC;
	}
}
...
bool CN3Mesh::Import(CN3PMesh* pPMesh)
{
	...
	// vertex, index buffer 만들기 [Korean comment]
	Create(PMeshInstance.GetNumVertices(), PMeshInstance.GetNumIndices());

	// vertex index buffer 복사 [Korean comment]
	...
	m_szName = pPMesh->m_szName; // 이름.. Name
	return true;
}
