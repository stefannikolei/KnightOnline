...
void CN3UIButton::Render()
{
	...
	for(UIListReverseItor itor = m_Children.rbegin(); m_Children.rend() != itor; ++itor)
	{
		...
		for(i = 0; i < NUM_BTN_STATE; i++) // 버튼의 구성 요소가 아닌지 보고.. Button
			if(pChild == m_ImageRef[i]) break;
		if(i >= NUM_BTN_STATE) pChild->Render(); // 버튼 차일드가 아니면 렌더링.. Rendering
	}
}
...
uint32_t CN3UIButton::MouseProc(uint32_t dwFlags, const POINT& ptCur, const POINT& ptOld)
{
	...
	if(false == IsIn(ptCur.x, ptCur.y))	// 영역 밖이면 [Korean comment]
	{
		...
		dwRet |= UI_MOUSEPROC_PREVINREGION;	// 이전 마우스 좌표는 영역 안이었다. [Korean comment]

		if (UI_STATE_BUTTON_DISABLE == m_eState) return dwRet;	// disable이면 그냥 리턴 [Korean comment]

		if(UISTYLE_BTN_NORMAL & m_dwStyle) // normal 버튼 이면 Button
		{
			SetState(UI_STATE_BUTTON_NORMAL);	// normal 상태로 Status
		}
		else if (UISTYLE_BTN_CHECK & m_dwStyle) // check 버튼 이면 Button
		{
			if (UI_STATE_BUTTON_DOWN_2CHECKUP == m_eState)	// up시키려다 만 경우  [Korean comment]
				SetState(UI_STATE_BUTTON_DOWN);	// down 상태로 Status
			else if (UI_STATE_BUTTON_DOWN_2CHECKDOWN == m_eState ||	// down시키려다 만 경우 또는 [Korean comment]
				UI_STATE_BUTTON_ON == m_eState)	// on 상태일 경우 Status
				SetState(UI_STATE_BUTTON_NORMAL);	// normal 상태로 Status
		}
		return dwRet; // 영역 밖이므로 더이상 처리 하지 않는다. Process
	}
	dwRet |= UI_MOUSEPROC_INREGION;	// 이번 마우스 좌표는 영역 안이다 [Korean comment]

	if (UI_STATE_BUTTON_DISABLE == m_eState) return dwRet;	// disable이면 그냥 리턴 [Korean comment]

	// 클릭 영역 밖이면 [Korean comment]
	...
	// 아래는 클릭 영역 안일때.. [Korean comment]
	// normal 버튼 이면 Button
	if(UISTYLE_BTN_NORMAL & m_dwStyle)
	{
		if(dwFlags & UI_MOUSE_LBCLICK)  // 왼쪽버튼 눌르는 순간 Button
		{
			SetState(UI_STATE_BUTTON_DOWN); // 누른 상태로 만들고.. Status
			if (m_pSnd_Click) m_pSnd_Click->Play();	// 사운드가 있으면 play 하기
			dwRet |= UI_MOUSEPROC_DONESOMETHING;
			return dwRet;
		}
		else if(dwFlags & UI_MOUSE_LBCLICKED) // 왼쪽버튼을 떼는 순간 Button
		{
		...
		{
			SetState(UI_STATE_BUTTON_ON); // On 상태로 만들고.. Status
			if (m_pSnd_On) m_pSnd_On->Play();	// 사운드가 있으면 play 하기
			dwRet |= CN3UIBase::MouseProc(dwFlags, ptCur, ptOld);
			return dwRet;
			// UI_MOUSEPROC_DONESOMETHING를 넣으면 안된다.(마우스 포인터가 버튼에서 다른 버튼으로 빠르게 옮겨갈때  Button
			// 이전 버튼의 상태가 이상해지는 것을 방지하기 위해) Button
		}
	}
	...
	else if(UISTYLE_BTN_CHECK & m_dwStyle) 
	{
		...
		else if(dwFlags & UI_MOUSE_LBCLICKED)  // 왼쪽버튼 떼는 순간 Button
		{
			if(UI_STATE_BUTTON_DOWN_2CHECKDOWN == m_eState) // 이전 상태가 2CHECKDOWN 상태이면
			{
				SetState(UI_STATE_BUTTON_DOWN); // down 상태로 만들기 Status
			...
			{
				SetState(UI_STATE_BUTTON_ON); // On 상태로 만들기 Status
				if (m_pParent) m_pParent->ReceiveMessage(this, UIMSG_BUTTON_CLICK); // 부모에게 버튼 클릭 통지.. Button
				dwRet |= UI_MOUSEPROC_DONESOMETHING;
				return dwRet;
			}
...
bool CN3UIButton::Load(HANDLE hFile)
{
	...
	DWORD dwNum;
	ReadFile(hFile, &m_rcClick, sizeof(m_rcClick), &dwNum, nullptr);	// click 영역 [Korean comment]

	// m_ImageRef 설정하기 Set
	for(UIListItor itor = m_Children.begin(); m_Children.end() != itor; ++itor)
	{
		CN3UIBase* pChild = (*itor);
		if (UI_TYPE_IMAGE != pChild->UIType()) continue;	// image만 골라내기 [Korean comment]
...
void CN3UIButton::operator = (const CN3UIButton& other)
{
	...
	m_rcClick = other.m_rcClick;			// 클릭 영역 [Korean comment]
	SetSndOn(other.GetSndFName_On());		// 사운드 [Korean comment]
	SetSndClick(other.GetSndFName_Click());	// 사운드 [Korean comment]
...
bool CN3UIButton::Save(HANDLE hFile)
{
	...
	DWORD dwNum;
	WriteFile(hFile, &m_rcClick, sizeof(m_rcClick), &dwNum, nullptr);	// click 영역 [Korean comment]
