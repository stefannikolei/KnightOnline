...
protected:
	...
	int				m_iMaxValue;					// 최대 [Korean comment]
	int				m_iMinValue;					// 최소 [Korean comment]
	float			m_fCurValue;					// 현재 값 - 부드럽게 점차 값을 올려가려고 float 로 했다..
	float			m_fChangeSpeedPerSec;			// 현재값이 변해야 되는 속도.. Unit SpeedPerSec
	int				m_iValueToReach;				// 도달해야 될값 - 부드럽게 값이 올라가는 경우에 필요하다.. [Korean comment]
	float			m_fTimeToDelay;					// 지연시간.. Time
...
public:
	...
	void			SetCurValue_Smoothly(int iValue, float fPercentPerSec){SetCurValue(iValue, 0, fPercentPerSec*(m_iMaxValue-m_iMinValue));}	// 초당 몇 퍼센트 수치로 올라간다. [Korean comment]
...
public:
	...
	void			DeleteBkGndImage();	// Back ground이미지는 필요 없는 경우가 있다. [Korean comment]
#endif
};
