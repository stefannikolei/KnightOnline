...
protected:
    __Vector3		m_vPickRayDir;			// 충돌 체크할 직선의 방향 Check
    __Vector3		m_vPickRayOrig;			// 충돌 체크할 직선의 시작점 Check

// Operations
public:
	BOOL PickByBox(__Vector3 &vMin, __Vector3 &vMax, __Vector3& vIntersect);
	void			SetPickXY(long x, long y); // 클라이언트 좌표를 넣어준다.. [Korean comment]

	// [in] v0, v1, v2 세점의 좌표 [Korean comment]
	...
	// v0 + (v1-v0)*u + (v2-v0)*v 를 구하면 만나는 점을 구할 수 있다. [Korean comment]
