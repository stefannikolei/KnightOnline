...
protected:
	...
	float			m_fScale;			// 화면 배율 [Korean comment]
	...
	BOOL			m_bDrag;			// 드래그 중인가? [Korean comment]
	CPen			m_WhiteDashPen;		// 흰색 점선 Color
	CPoint			m_ptMouseOld;		// 마우스의 이전 지점 기억 [Korean comment]
	BOOL			m_bDeselect;		// deselect 할 것인가? [Korean comment]
	...
	int				m_iImageTypeCount;								// 선택되어야 할 이미지 종류의 수 Type
...
public:
	...
	void			Render();				// texture render하기 [Korean comment]
	void			SetTexture(LPCTSTR pszFName);	// texture 지정 [Korean comment]
	eEDITMODE		SetEditMode(eEDITMODE eMode);	// mode 바꾸기 (zoom, hand, select) 실패하면 이전 mode를 돌려준다.
	void			SetLeftTopInImage(CPoint ptLeftTop);	// 이미지의 좌측 상단 좌표 바꾸기 [Korean comment]
	...
	// image type관련 [Korean comment]
	void			SetImageTypeCount(int iCount) {m_iImageTypeCount = iCount;}	// image type 갯수 정하기 [Korean comment]
...
protected:
	...
	void			ProcessDrag(CPoint point);		// 영역 변형일 경우 처리하는 루틴 Process
public:
// Overrides
