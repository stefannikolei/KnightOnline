...
void CUIEDoc::OnInsertImage()
{
	...
	if (FALSE == SetImageInfos(pUI))
	{
		if (IDYES
			== pFrm->MessageBox(
				"Image 정보 지정이 취소되었습니다.\n생성된 이미지를 삭제하시겠습니까?", nullptr,
				MB_YESNO))
			OnEditDelete();
		else
...
void CUIEDoc::OnInsertProgress()
{
	...
	if (IDCANCEL == dlg.DoModal())
	{
		if (IDYES
			== pFrm->MessageBox(
				"텍스쳐 UV좌표 지정이 취소되었습니다.\n생성된 Progress를 삭제하시겠습니까?",
				nullptr, MB_YESNO))
			OnEditDelete();
		else
...
void CUIEDoc::OnInsertTrackbar()
{
	...
	if (FALSE == SetTrackBarInfos(pUI))
	{
		if (IDYES
			== pFrm->MessageBox(
				"Trackbar 정보 지정이 취소되었습니다.\n생성된 Trackbar를 삭제하시겠습니까?",
				nullptr, MB_YESNO))
			OnEditDelete();
		else
...
void CUIEDoc::OnInsertScrollbar()
{
	...
	if (FALSE == SetTrackBarInfos(pUITrackBar))
	{
		if (IDYES
			== pFrm->MessageBox(
				"Trackbar 정보 지정이 취소되었습니다.\n생성된 ScrollBar 삭제하시겠습니까?", nullptr,
				MB_YESNO))
			OnEditDelete();
		else
...
BOOL CUIEDoc::SetStringInfos(CN3UIString* pUI)
{
	...
	D3DCOLOR d3dColor = 0xff000000 | ((color & 0x00ff0000) >> 16) | (color & 0x0000ff00)
						| ((color & 0x000000ff) << 16);
...
void CUIEDoc::OnFileExportTooltip()
{
	...
	if (nullptr == pUI)
		return;
	ASSERT(UI_TYPE_STATIC
		   == pUI->UIType()); // tooltip은 static에서 상속받고 따로 저장하는 정보가 없다.
...
void CUIEDoc::OnEditMakeGroup()
{
		...
		else // 모두 같은 패어런트인지 확인..
		{
			if (pUIFirst->GetParent() != pUI->GetParent())
			{
				MessageBox(AfxGetMainWnd()->m_hWnd,
					"그룹시킬 UI 는 모두 같은 패어런트에 있어야 합니다.", "그룹지정 실패", MB_OK);
				return;
			}
		}
	}
	...
	if (iUIC <= 1)
	{
		MessageBox(AfxGetMainWnd()->m_hWnd, "두개이상의 UI 를 선택해야 그룹을 만들수 있습니다.",
			"그룹지정 실패", MB_OK);
		return;
	}
...
void CUIEDoc::OnEditUnparent()
{
	...
	{
		CN3UIBase* pUI = *it;
		if (pUI->GetParent() == nullptr || pUI->GetParent() == &(m_RootUI))
			continue;
