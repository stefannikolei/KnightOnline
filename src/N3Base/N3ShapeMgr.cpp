...
bool CN3ShapeMgr::Save(File& file)
{
	...
	for (CN3Shape* pShape : m_Shapes)
	{
		...
		if (pShape->m_iEventID != 0 || pShape->m_iEventType != 0 || pShape->m_iNPC_ID != 0
		...
		for (int j = 0; j < 3; j++)
		{
			...
			for (int z = zz1; z <= zz2; z++)
			{
				...
				for (int x = xx1; x <= xx2; x++)
				{
					...
					{
						// 위의 변과의 교차점을 계산하고..
						float fXCross = vEdge[j][0].x
										+ (fZMax - vEdge[j][0].z) * (vEdge[j][1].x - vEdge[j][0].x)
											  / (vEdge[j][1].z - vEdge[j][0].z);
						if (fXCross < fXMin)
					...
					if (!bWriteID)
					{
						...
						for (float fZ = zz3; fZ <= zz4 && !bWriteID; fZ += 0.25f)
						{
							for (float fX = xx3; fX <= xx4 && !bWriteID; fX += 0.25f)
							{
								...
								// 폴리곤 충돌 체크..
								if (::_IntersectTriangle(vPos, vDir, m_pvCollisions[i * 3],
										m_pvCollisions[i * 3 + 1], m_pvCollisions[i * 3 + 2]))
...
#ifdef _N3TOOL
bool CN3ShapeMgr::AddCollisionTriangle(
	const __Vector3& v1, const __Vector3& v2, const __Vector3& v3)
{
...
void CN3ShapeMgr::RenderCollision(const __Vector3& vPos)
{
	...
	for (int i = 0; i < 9; i++)
	{
		if (ppCell[i] == nullptr || ppCell[i]->nCCPolyCount <= 0)
			continue;
		...
		for (int j = 0; j < nFC; j++)
		{
			...
			vDir.Cross((m_pvCollisions[n1] - m_pvCollisions[n0]),
...
CN3Shape* CN3ShapeMgr::Pick(int iXScreen, int iYScreen, bool bMustHaveEvent, __Vector3* pvPick)
{
	...
	{
		// 이벤트가 있어야 한다면...
		if (bMustHaveEvent && pShape->m_iEventID <= 0)
			continue;
