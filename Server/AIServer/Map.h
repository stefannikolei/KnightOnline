...
class CMapInfo					// 각 좌표의 정보 Info
{
public:
	short	m_sEvent;			// 현좌표의 이벤트 번호 [Korean comment]
...
public:
	...
	CMapInfo**			m_pMap;				// 타일의 정보(1셀 : 4미터) Info
	CRegion**			m_ppRegion;			// 64미터의 타일정보.. Info
	//CRoomEvent*		m_pRoomEvent;
	CSize				m_sizeMap;			// 맵의 크기 Size
...
//	short				m_arDungeonBossMonster[MAX_DUNGEON_BOSS_MONSTER];
	BYTE				m_byRoomType;		// 방의 초기화관련( 0:자동으로 초기화, 1:전쟁이벤트 관련(특정조건이 완료시 초기화) Initialize
	BYTE				m_byRoomEvent;		// event room(0:empty, 1:use)
	BYTE				m_byRoomStatus;		// room status(1:진행중, 2:방을 초기화중, 3:방초기화 완료) Initialize
	BYTE				m_byInitRoomCount;	// room 초기화 시간을 제어(몬스터와 동기화를 맞추기 위해) Initialize
	ObjectEventArray	m_ObjectEventArray;
	RoomEventArray		m_arRoomEventArray;
	short				m_sKarusRoom;		// karus의 성갯수 [Korean comment]
	short				m_sElmoradRoom;		// elmorad의 성갯수 [Korean comment]
	...
	int IsRoomCheck(float fx, float fz);	// 던젼에서 사용, 유저의 현재위치가 던젼의 어느 위치에 있는지를 판단 Position
