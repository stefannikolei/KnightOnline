...
var sql = builder.AddSqlServer("sqlserver", sqlPassword, port: 1433)
...
builder.Services
    .AddHealthChecks()
    .AddCheck("kn-online-schema", new SchemaHealthCheck(
        ct => ((IResourceWithConnectionString)gameDb.Resource).GetConnectionStringAsync(ct)));
...
var kodb = builder.AddDockerfile("kodb-util", "../../docker/kodb-util")
    ...
    .WithArgs("-c",
        "if [ ! -f /var/lib/app/.openko-imported ]; then " +
        "/usr/local/bin/cleanImport.sh && touch /var/lib/app/.openko-imported; fi; " +
        "exec sleep infinity")
    .WaitFor(sql)
    ...
    .WithCommand(
        ...
        new CommandOptions
        {
            ...
            ConfirmationMessage = "This DROPS the KN_online database and re-imports it from scratch. Continue?",
            IconName = "ArrowReset",
        })
    .WithCommand(
        "reload-schema",
        ...
        new CommandOptions
        {
            ...
            ConfirmationMessage = "This pulls the latest schema and re-imports KN_online. Continue?",
