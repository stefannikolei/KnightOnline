...
| 4 | Ebenezer (WIZ_CRYPTION handshake, LZF packets, game logic) | in progress — socket layer, login/char flow, GAMESTART, region world (4.1–4.4), AISocket link/NPC mirror sync (4.5), combat (4.6), magic (4.7), respawn/warp (4.8), chat/inventory (4.9), parties + SMD map loading (4.10), warehouse (4.11) and the trade/zone slice (4.12: WIZ_EXCHANGE with the inventory-mirror flow, ZoneChange + WIZ_WARP_LIST/WIZ_VIRTUAL_SERVER warp gates, stat/skill points + WIZ_CLASS_CHANGE resets, object events [bind points, gate levers, battle flags], WIZ_HOME via START_POSITION, speed-hack check, GM kick/user counts, WIZ_DATASAVE) done; plus knights/clans (4.13), the quest VM (4.14: QUESTS/<zone>.evt parser, EVENT/EXEC/LOGIC interpreter, WIZ_CLIENT_EVENT/WIZ_SELECT_MSG/WIZ_EDIT_BOX, EVENT_TRIGGER) and the battle events + GM slice (4.15: AG_BATTLE_EVENT, BattleZoneOpenTimer war countdown, game-time/weather tick, WIZ_OPERATOR + the '+' OperationMessage commands) done; only the party/market BBS boards remain open |
