...
| 4 | Ebenezer (WIZ_CRYPTION handshake, LZF packets, game logic) | in progress — socket layer, login/char flow, GAMESTART, region world (4.1–4.4), AISocket link/NPC mirror sync (4.5), combat (4.6), magic (4.7), respawn/warp (4.8), chat/inventory (4.9), parties + SMD map loading (4.10), warehouse (4.11) and the trade/zone slice (4.12: WIZ_EXCHANGE with the inventory-mirror flow, ZoneChange + WIZ_WARP_LIST/WIZ_VIRTUAL_SERVER warp gates, stat/skill points + WIZ_CLASS_CHANGE resets, object events [bind points, gate levers, battle flags], WIZ_HOME via START_POSITION, speed-hack check, GM kick/user counts, WIZ_DATASAVE) done; plus knights/clans (4.13: WIZ_KNIGHTS_PROCESS create/join/withdraw/destroy/rank changes through the Aujard library, KNIGHTS/KNIGHTS_USER startup cache, clan blocks in the user-info/my-info packets) done; quests/battle events and the remaining slices open |
