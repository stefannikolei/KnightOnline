...
| 4 | Ebenezer (WIZ_CRYPTION handshake, LZF packets, game logic) | in progress — socket layer, login/char flow, GAMESTART, region world (4.1–4.4), AISocket link/NPC mirror sync (4.5), combat (4.6), magic (4.7), respawn/warp (4.8), chat/inventory (4.9) and parties + SMD map loading (4.10: WIZ_PARTY flow, loot routing/gold split, party status broadcasts, real map extents/object events from Server/bin/MAP) done; plus the warehouse (4.11: WIZ_WAREHOUSE bank/deposit/withdraw) done; knights/quests/exchange and the remaining slices open |
